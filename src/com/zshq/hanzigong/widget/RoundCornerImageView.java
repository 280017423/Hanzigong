...
public class RoundCornerImageView extends ImageView {
	...
	private void drawLiftDown(Canvas canvas) {
		...
		path.arcTo(new RectF(0, getHeight() - mRoundHeight * 2, 0 + mRoundWidth * 2, getHeight()), 90, 90);
		path.close();
		canvas.drawPath(path, mPaint);
	}
	...
	private void drawRightUp(Canvas canvas) {
		...
		path.arcTo(new RectF(getWidth() - mRoundWidth * 2, 0, getWidth(), 0 + mRoundHeight * 2), -90, 90);
		path.close();
		canvas.drawPath(path, mPaint);
	}

	public void setRoundWidth(int roundWidth) {
		this.mRoundHeight = roundWidth;
	}
